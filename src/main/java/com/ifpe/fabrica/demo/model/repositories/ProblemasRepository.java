package com.ifpe.fabrica.demo.model.repositories;
...
public class ProblemasRepository implements Repository<Problema, Integer> {
   ...
   public Problema read(Integer codigo) throws SQLException {
      ...
      if (rs.next()) {
         ...
         return new Problema(rs.getInt("codigo"), rs.getString("tipo"), rs.getTimestamp("data").toLocalDateTime(),
               funcionario, setor);
      }
      return null;
   }
   ...
   public List<Problema> findAll() throws SQLException {
      ...
      while (rs.next()) {
         Funcionario funcionario = new FuncionarioRepository().read(rs.getInt("funcionario_codigo"));
         Setor setor = new SetorRepository().read(rs.getInt("setor_codigo"));
         problemas.add(new Problema(rs.getInt("codigo"), rs.getString("tipo"),
               rs.getTimestamp("data").toLocalDateTime(), funcionario, setor));
      }
      return problemas;
   }
}